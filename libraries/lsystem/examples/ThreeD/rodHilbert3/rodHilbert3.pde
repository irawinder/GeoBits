 ...
 * See version2 of this sketch for the new Turtle3D interface and implementing class RodTurtle
 */
...
void keyReleased() {
  switch(key) {
  case '+':
    if (depth <= 4) { // guard against a depth we can't handle
      depth++;
      distance = 300;
      setupGrammar();
    }
